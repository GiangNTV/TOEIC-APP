...
//   Architecture: fetch() + JSON data files
//   No inline JS data files needed.
// ═══════════════════════════════════════════════
...
const App = (() => {
  ...
  async function init() {
    try {
      setLoadProgress(10, 'Đang tải từ vựng...');
      const [vocabRes, questionsRes, grammarRes] = await Promise.all([
        fetch('data/vocab.json'),
        fetch('data/questions.json'),
        fetch('data/grammar.json'),
      ]);

      setLoadProgress(50, 'Đang xử lý dữ liệu...');
      DB.vocab    = await vocabRes.json();
      DB.questions= await questionsRes.json();
      DB.grammar  = await grammarRes.json();

      // Flatten questions for quiz use (add part tag to each question)
      flatQuestions = [];
      ...
      document.getElementById('load-msg').textContent = '❌ Lỗi tải dữ liệu. Hãy chạy app qua server (không mở file trực tiếp).';
  ...
  function generateUnitQuiz(unitId) {
    ...
    let pool = flatQuestions.filter(q => {
      ...
      return keywords.some(k => text.includes(k)) || q.type === grammar || (prevGrammar && q.type === prevGrammar);
    });

    if (pool.length < 10) {
      const extra = flatQuestions.filter(q => (q.part === 5 || q.part === 6) && !pool.includes(q));
      shuffleArray(extra);
      pool.push(...extra.slice(0, 15 - pool.length));
    }
